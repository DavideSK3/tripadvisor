...
CREATE TABLE ORARIO (
	...
	apertura TIME,
	chiusura TIME,
	CONSTRAINT DAY_CHECK CHECK (giorno >= 1 AND giorno <= 7),
...
CREATE TABLE REVIEWS (
	...
	FOOD INTEGER CONSTRAINT REVIEW_FOOD_CHECK 
		    CHECK (FOOD IS NULL OR (FOOD>= 1 AND FOOD <=5)),
	...
	SERVICE INTEGER CONSTRAINT REVIEW_SERVICE_CHECK 
		       CHECK (SERVICE IS NULL OR (SERVICE >= 1 AND SERVICE <= 5)),
	VALUE_FOR_MONEY INTEGER CONSTRAINT REVIEW_VALUE_FOR_MONEY_CHECK 
		        CHECK (VALUE_FOR_MONEY IS NULL OR(VALUE_FOR_MONEY >= 1 AND VALUE_FOR_MONEY <= 5)),
...
CREATE TABLE RESTAURANTS_CUISINES (
    ...
    FOREIGN KEY(ID_CUISINE) REFERENCES CUISINES(ID) ON DELETE CASCADE
